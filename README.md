...
.hero-fg-video {
  ...
  -webkit-mask-image: radial-gradient(ellipse 70% 80% at 50% 45%, black 50%, transparent 100%);
  mask-image: radial-gradient(ellipse 70% 80% at 50% 45%, black 50%, transparent 100%);
  opacity: 0.9;
}
.hero-grade {
  position: absolute; inset: 0; pointer-events: none;
  background: 
    radial-gradient(ellipse 60% 50% at 50% 30%, rgba(217, 138, 79, 0.15), transparent 60%),
    linear-gradient(to bottom, rgba(8,8,10,0.4) 0%, rgba(8,8,10,0.2) 40%, rgba(8,8,10,0.95) 100%);
}
.glass-btn {
  width: 44px; height: 44px; border-radius: 50%;
  background: rgba(18, 17, 21, 0.6);
...
.glass-btn:hover {
  background: rgba(217, 138, 79, 0.25);
  border-color: #d98a4f;
...
<section id="hero" class="relative min-h-screen flex items-center justify-center pt-24 pb-16 overflow-hidden">
  <!-- Hero Background & Avatar Video -->
  <video class="hero-bg-video" src="prasad-hero.mp4" autoplay loop muted playsinline aria-hidden="true"></video>
  <video class="hero-fg-video" id="fgVideo" src="prasad-hero.mp4" autoplay loop muted playsinline aria-label="Prasad Sudhir Thorat interactive introduction video"></video>
  
  <div class="hero-grade"></div>

  <!-- 3D Three.js WebGL Interactive Particle Canvas -->
  <canvas id="hero3dCanvas" class="absolute inset-0 z-10 w-full h-full pointer-events-auto"></canvas>
  ...
  <div class="absolute bottom-6 right-6 z-30 flex items-center gap-3">
    <button class="glass-btn text-white" id="videoPlayBtn" aria-label="Pause introduction video" title="Play/Pause Video">
      <svg id="videoPlayIcon" class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><rect x="6" y="5" width="4" height="14"/><rect x="14" y="5" width="4" height="14"/></svg>
    </button>
    <button class="glass-btn text-white" id="videoMuteBtn" aria-label="Unmute video audio" title="Mute/Unmute Video Audio">
      <svg id="videoMuteIcon" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2"></path></svg>
    </button>
  </div>
  ...
  <div class="max-w-4xl mx-auto space-y-12 text-center">
    ...
    <form id="contactForm" onsubmit="handleContactSubmit(event)" class="glass-panel p-8 rounded-2xl border border-neutral-800 text-left space-y-4 max-w-xl mx-auto">
      <div class="space-y-1">
        ...
        <input type="text" id="senderName" required placeholder="John Doe" 
      ...
      <div class="space-y-1">
        ...
        <input type="email" id="senderEmail" required placeholder="john@example.com" 
               class="w-full px-4 py-2.5 rounded-xl bg-neutral-900/90 text-xs font-mono text-white border border-neutral-800 focus:outline-none focus:border-amber-500" />
      </div>
      <div class="space-y-1">
        <label class="font-mono text-xs text-neutral-400 uppercase">Message</label>
        <textarea id="senderMsg" rows="4" required placeholder="Hello Prasad..." 
      ...
      <button type="submit" class="w-full py-3 rounded-xl bg-accent-dynamic text-black font-mono font-bold text-xs uppercase tracking-wider hover:opacity-90 transition-all flex items-center justify-center gap-2">
        <span>Send Direct Message to WhatsApp</span>
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
      </button>
...
if (videoPlayBtn && fgVideo) {
  videoPlayBtn.addEventListener('click', () => {
    if (fgVideo.paused) {
      fgVideo.play();
      bgVideos.forEach(v => v.play());
      document.getElementById('videoPlayIcon').innerHTML = '<rect x="6" y="5" width="4" height="14"/><rect x="14" y="5" width="4" height="14"/>';
    } else {
      fgVideo.pause();
      bgVideos.forEach(v => v.pause());
      document.getElementById('videoPlayIcon').innerHTML = '<polygon points="5,3 19,12 5,21"/>';
    }
  });
}

if (videoMuteBtn && fgVideo) {
  videoMuteBtn.addEventListener('click', () => {
    fgVideo.muted = !fgVideo.muted;
    if (!fgVideo.muted) {
      fgVideo.play().catch(() => {});
    }
    document.getElementById('videoMuteIcon').innerHTML = fgVideo.muted
...
// CONTACT FORM HANDLING - DIRECT WHATSAPP DISPATCH TO 8010989708
function handleContactSubmit(e) {
  ...
  const targetPhoneNumber = "918010989708";
  const formattedText = `Hello Prasad,\n\nName: ${name}\nEmail: ${email}\n\nMessage:\n${msg}`;
  const whatsappUrl = `https://wa.me/${targetPhoneNumber}?text=${encodeURIComponent(formattedText)}`;

  feedback.textContent = "Redirecting your message to Prasad on WhatsApp (+91 8010989708)...";
